...
{
    int size = my_strlen(tab[1]) + my_strlen(tab[2]) + 1;
    ...
    new_env[a] = malloc(sizeof(char) * (size + 1));
    if (new_env[a] == NULL)
        exit(84);
    new_env[a] = my_strcpy(new_env[a], re_mal(re_mal(tab[1], "="), tab[2]));
    a++;
    new_env[a] = NULL;
    free_tab((*env));
    (*env) = new_env;
...
{
    char *str = re_mal(re_mal(tab[1], "="), tab[2]);
